...
public class CopyUtil {
	...
	public static void search(int now, int length, File source_files[], File dest_file[]) {
		if (length == 0)
			return;
		else if (now >= length)
			return;
		...
		if (source_files[now].isDirectory()) {
			...
			dest_file[now].mkdir();
			search(0, this_files.length, this_files,dest_file[now].listFiles());
		}

		else if (source_files[now].isFile()) {
			try {
				copyFile(source_files[now], new File(dest_file[now].getPath(), source_files[now].getName()));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
